...
def select_movie():
    while True:
        movie_choice = input("A fenti filmek közül melyikre szeretnél jegyet foglalni? ").strip().upper()
        if movie_choice in movies:
            print(f"{movie_choice}? Mi is imádjuk, és pont játsszuk.")
            return movie_choice
        else:
            print("A fenti listából válassz!")
...
def book_seats(num_tickets):
    ...
    for _ in range(num_tickets):
        while True:
            try:
                row = int(input("Melyik sor? (1-5): ")) - 1  # 0-4 helyett 1-5 indexelés
                col = int(input("Hányas szék? (1-5): ")) - 1  # 0-4 helyett 1-5 indexelés
                
                # Ellenőrizzük, hogy a megadott sor és oszlop érvényes-e
                if row < 0 or row >= 5 or col < 0 or col >= 5:
                    print("Ilyen hely nincs. Válassz létező helyet!")
                    continue

                # Ellenőrizzük, hogy a hely foglalt-e
                if seats[row][col] == "F":
                    print("Mások ölébe nem ülhetsz! Válassz másik helyet.")
                else:
                    # Foglalás végrehajtása
                    seats[row][col] = "F"  # Lefoglalt hely "F" jelöléssel
                    booked_seats.append((row + 1, col + 1))  # A visszatérő érték legyen 1-től 5-ig
                    display_seats()  # Frissített ülésrendszer kiírása
                    break
            except ValueError:
                print("►Hiba: Kérlek adj meg 1-5-ig számot a sor és oszlop megadásánál.")

    return booked_seats

# Program kezdete
def main():
    print("Örülök, hogy itt vagy! Válassz egyet a kiváló filmek közül:")
    print(", ".join(movies))
    ...
    while True:
        try:
            num_tickets = int(input("Hányan jöttök? (Max. 25 hely van.)"))
            if num_tickets > 0 and num_tickets <= 25:
                break
            else:
                print("Maximum 25-en fértek el, és egynél kevesebben nem tudsz jönni.")
        except ValueError:
            print("Számot adj meg 1-25 között!")
