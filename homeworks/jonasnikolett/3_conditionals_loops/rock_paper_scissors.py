...
player_1 = input("Player 1 nickname: ").capitalize()
player_2 = input("Player 2 nickname: ").capitalize()

#alap adatok
while True:
    game_round = int(input("How many rounds do you want to play? Please add odd number "))
    if game_round < 3 or game_round%2 == 0:
        print("Invalid input, please define a positive odd number which grater than 2 (you don't play draw in the game)")
    else:
        print("Thanks, valid input.")
        break

#innentől maga a játék folyamata

attempts = 0
p1_point = 0
p2_point = 0
while attempts < game_round:
    answer_player_1 = input(f"{player_1} enter your choice in the {attempts+1} round: /rock, paper or scissors/ ").lower().strip()
    answer_player_2 = input(f"{player_2} enter your choice in the {attempts+1} round: /rock, paper or scissors/ ").lower().strip()
