...
from models import HouseholdAppliances, HouseholdAppliancesResponse, HouseholdAppliancesResponse
...
class HouseholdAppliancesRequest:
    item_name: str
    quantity: int
    price: float
...
async def create_product(product: HouseholdAppliancesRequest, db: AsyncSession = Depends(get_db)):
    new_product = HouseholdAppliances(id=str(uuid.uuid1()), **product.dict())
    db.add(new_product)
    await db.commit()
